    ...
    def process(self, images, chunk_size: int):
        ...
        try:
            if self.DEBUG:
                ...
                print(f"  N={n}, chunk_size={chunk_size}, fps={self.FPS_FALLBACK}, size={w}x{h}")
                print(f"  save_individual_chunks={self.SAVE_INDIVIDUAL_CHUNKS}, head_win={self.HEAD_WIN}, tail_win={self.TAIL_WIN}")

            out_final = self._ensure_mp4v_writer(final_path_tmp, self.FPS_FALLBACK, w, h)
